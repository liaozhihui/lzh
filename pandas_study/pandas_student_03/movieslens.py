...
data_gender=pd.pivot_table(data,index="Title",columns="Gender",values='Rating',aggfunc='mean')
print(data_gender.head())
# 使用crosstab方法 查看每一部电影不同性别的平均评分
data_gender = pd.crosstab(index=data.Title,columns=data.Gender,values=data.Rating,aggfunc='mean')
print(data_gender.head())