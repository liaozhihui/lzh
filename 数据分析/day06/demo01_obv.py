...
sign_prices = np.piecewise(diff_prices,[diff_prices>0,diff_prices<0,diff_prices==0],[1,-1,0])
...
dates=dates[1:]
volumns=volumns[1:]
mp.bar(dates[sign_prices==1],volumns[sign_prices==1],0.8,color='red',label='OBV')
mp.bar(dates[sign_prices==-1],volumns[sign_prices==-1],0.8,color='green',label='OBV')
mp.legend()
